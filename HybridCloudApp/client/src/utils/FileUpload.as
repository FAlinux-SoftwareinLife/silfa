	...
	public class FileUpload extends EventDispatcher {
		...
		private function constructPostDataAsync(): void {
			...
			if (_fileNames.length) {
				...
				_prepared = true;
				// dispatchEvent( new FileUploadEvent(FileUploadEvent.DATA_PREPARE_COMPLETE) );
			}
		}
		...
		private function nextAsyncLoop(): void {
			...
			if (asyncFilePointer < _fileNames.length) {
				...
				//				dispatchEvent( new FileUploadEvent(FileUploadEvent.DATA_PREPARE_PROGRESS, totalFilesSize, totalFilesSize) );
				//				dispatchEvent( new FileUploadEvent(FileUploadEvent.DATA_PREPARE_COMPLETE) );
			}
		}
		...
		private function writeChunkLoop(dest: ByteArray, data: ByteArray, p: uint = 0): void {
			...
			if (writtenBytes % BLOCK_SIZE * 2 == 0) {
				// dispatchEvent( new FileUploadEvent(FileUploadEvent.DATA_PREPARE_PROGRESS, writtenBytes, totalFilesSize) );
			}
